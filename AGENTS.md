...
**hukitori** は、複数ユーザーが同時に「こする」操作を共有し、隠れた 4 桁の数字を当てるリアルタイム Web ゲームです。
...
4. **画像生成はサーバー側** — 4 桁表示・ピクセル配列は `generate-image.js` を単一の正とする。
...
- `index.html` 内の `if (1) { ... _debugloop ... }` はデバッグ用の自動こすりループ。本番挙動変更時は意図を確認すること。
